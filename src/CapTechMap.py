...
@app.route('/')
# This function will create the map and return it
def index():
    # These are the coordinates for the university, based on latitude and longitude
    start_coords = (39.046900, -76.850400)
    # We then create a normal map of the Universities campus
    map_cap = folium.Map(location=start_coords, width="100%", zoom_start=17)

    #  Here we create an outline for the feild, and display it
    feild_cap = 'field.geojson'
    display(folium.GeoJson(feild_cap, name="Feild").add_to(map_cap))

    #  Here we create an outline for the basketball court, and display it
    basketball_cap = 'basketball.geojson'
    display((folium.GeoJson(basketball_cap, name="BasketBall").add_to(map_cap)))

    #  Here we create an outline for the McGowan building, and display it
    mcgowan_cap = 'McGowan.geojson'
    display((folium.GeoJson(mcgowan_cap, name="McGowan").add_to(map_cap)))

    # Here we create an outline for the building that houses the Megabyte, and display it
    ComHall_cap = 'MA.geojson'
    display((folium.GeoJson(ComHall_cap, name="Megabyte").add_to(map_cap)))

    # Here we create an outline for the pond, and display it
    ...
    # Here we create an outline for the library and display it
    library_cap = 'library.geojson'
    display((folium.GeoJson(library_cap, name="Library").add_to(map_cap)))

    # Here we create an outline for the Auditorium and display it
    aud_cap = 'Auditorium.geojson'
    display((folium.GeoJson(aud_cap, name="Auditorium").add_to(map_cap)))

    # Here we create an outline for the Innovators Hall and display it
    innovators_cap = 'innovators.geojson'
    display((folium.GeoJson(innovators_cap, name="Innovators").add_to(map_cap)))


    # The following lines of code place a marker in the specefied location with the specefic icon for field
    ...
    # The following lines of code place a marker in the specefied location with the specefic icon for a certain location where there is sometimes a campfire
    ...
    # The following lines of code place a marker in the specefied location with the specefic icon for a place where students tend to play their instruments
    ...
    # The following lines of code place a marker in the specefied location with the specefic icon for the cyber battle lab
    ...
    # The following code place a marker in the specefied location with the specefic icon for the megabyte cafe
    ...
    # The following code place a marker in the specefied location with the specefic icon for where students to play fighting games throughout the day
    ...
    # The following code place a marker in the specefied location with the specefic icon for the pond
    ...
    # The following code place a marker in the specefied location with the specefic icon for the Esports Lab
    ...
    # The following code place a marker in the specefied location with the specefic icon for where one of the pool tables are located
    ...
    # The following code place a marker in the specefied location with the specefic icon where students tend to fly their drones
    ...
    # The following code place a marker in the specefied location with the specefic icon for the pool table in the innovators hall
    folium.Marker(location=[39.04537766001136, -76.85210645198822],
                  popup='One of our two pool table locations where students can enjoy a nice releaxing game of pool.',
                  icon=folium.features.CustomIcon('pool_table.png', icon_size=(14, 14))).add_to(map_cap)

    # The following code place a marker in the specefied location with the specefic icon for the library
    ...
    # The following code place a marker in the specefied location with the specefic icon for the place where casino night is held in innovators
    ...
    # The following code place a marker in the specefied location with the specefic icon for the place where the adventure club meets prior to departing for events
    ...
    folium.plugins.AntPath([[39.0454068238309, -76.85056149959563], [39.04540265757169, -76.85084581375122],
        ...
        map_cap)
    # We then return the updated map
